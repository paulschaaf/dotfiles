...
//  def descendingByRetweet0(acc: Trending): Trending
...
//  def descendingByRetweet0(acc: Trending): Trending = acc
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def ascendingByRetweet0(acc: Trending): Trending = {
    val min = findMin
    remove(min).ascendingByRetweet0(acc + min)
  }
//  def descendingByRetweet0(acc: Trending): Trending = {
//    //val min = findMax
//    remove(min).ascendingByRetweet0(acc + min)
...
object GoogleVsApple {
  ...
  val appleTweets: TweetSet = {
    TweetReader.allTweets.filter(tw => apple.exists(str => tw.text.contains(str)))
  }
