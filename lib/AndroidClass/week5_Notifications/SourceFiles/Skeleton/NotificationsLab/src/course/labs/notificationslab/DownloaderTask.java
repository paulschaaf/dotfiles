...
public class DownloaderTask extends AsyncTask<String, Void, String[]> {
   ...
   private void notify(final boolean success) {
      ...
      final Intent restartMainActivtyIntent = new Intent(mParentActivity, MainActivity.class);
      ...
      mApplicationContext.sendOrderedBroadcast(
            ...
            new BroadcastReceiver() {
               ...
               public void onReceive(Context context, Intent intent) {
                  ...
                  // T_ODO: Check whether the result code is RESULT_OK
                  if (getResultCode() != Activity.RESULT_OK) {
                     // T_ODO:  If so, create a PendingIntent using the
                     ...
                     // Uses R.layout.custom_notification for the layout of the notification View. The xml file is in res/layout/custom_notification.xml
                     RemoteViews mContentView = new RemoteViews(
                           mApplicationContext.getPackageName(),
                           R.layout.custom_notification);

                     // T_ODO: Set the notification View's text to reflect whether or the download completed successfully
                     mContentView.setTextViewText(
                           R.layout.custom_notification,
                           success ? successMsg : failMsg);
                     
                     // T_ODO: Use the Notification.Builder class to create the Notification. You will have to set several pieces of
                     // information. You can use android.R.drawable.stat_sys_warning for the small icon. You should also setAutoCancel(true). 
                     Notification.Builder notificationBuilder = new Notification.Builder(mApplicationContext)
                        //.setTicker(tickerText)
                        .setSmallIcon(android.R.drawable.stat_sys_warning)
                        .setAutoCancel(true)
                        .setContentTitle(TAG)
                        .setContentText(success ? successMsg : failMsg)
                        .setContentIntent(pendingIntent);

                     // T_ODO: Send the notification
