...
class ListsSuite extends FunSuite {
  ...
  test("sum of a few numbers")    { assert(sum(List(1,2,0))      ===  3) }
  test("sum of negative numbers") { assert(sum(List(-1, -2, -3)) === -6) }
  
  test("max of an empty list")    {
    intercept[NoSuchElementException] { max(List()) }
  }
  test("max of a few numbers")    { assert(max(List(3, 7, 2))    ===  7) }
  test("max of negative numbers") { assert(max(List(-3, -7, -2)) === -2) }
}
