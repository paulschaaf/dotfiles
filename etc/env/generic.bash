...
# Built:	Wed Sep 22 22:19:17 PDT 2004
# By:  	pschaaf@copernicus
# For:
#    host     = generic      shell    = bash      
...
# ======== Completion, Hashing
if [ -n "$interactive" ]; then
set -o hashall; shopt -s checkhash
shopt -s dotglob extglob
set completion-ignore-case on; shopt -s nocaseglob
...
pushd () {
	# if target is a file, change to its location
         if [ -f $1 ]; then
           builtin pushd $(dirname $1) > /dev/null
         else
...
# ======== Shell History
if [ -n "$interactive" ]; then
set HISTCONTROL=ignoreboth hist_ignore_dups
...
# ======== Cygwin Path
# ======== Function Keys
bind '"[17~":kill-whole-line'	# F6 cut
bind '"[18~":copy-region-as-kill'	# F7 copy
bind '"[19~":yank'	# F8 paste
bind '"[20~":history-incremental-search-backward'	# F9 search
if [ -z "$inScreen" ]; then
bind -x '"[24~":"exit\n"'	# F12 exit
fi
# ======== Cursor Keys
bind '"[1;5D":backward-word'	# ctrl_left
bind '"[1;5C":forward-word'	# ctrl_right
# ======== Other Extra Keys
bind '"[3~":delete-char'	# delete
...
# ======== Macro Keys
bind -x '""^_"":"\C-apath \C-e\C-m"'	# C-? prepends 'whence' to command line
# ======== Lesskey
LESSKEY=/home/pschaaf/.less_ftp; export LESSKEY
# ======== Classpath
# ======== Path
...
# ======== Prompt
if [ -n "$interactive" ]; then
HOST_COLOR="[1;41;37m"; export HOST_COLOR
if [ "$UID" = "0" ]; then
prependToVar HOST_COLOR "[41m### "
fi
if [ -n "$inScreen" ]; then
PROMPT_COMMAND='echo -n -e "k\134"'
fi
dirs_status () {
	source $HOME/lib/sh/dirs_status
}
...
chpwd () {
	local dirs
       ...
       awk 'NR == 2 {printf("%s", "")}
            {print}' ORS=' '`"
PS1="$CTVIEW
`date +%R` [1;40;32m\u@\h $dirs
\[\]\!\[\] \[${CONNECTION_COLOR}\]${CONNECTION}\[\]${CONNECTION_END}${SCRIPTING_MSG}b\\$ "
}
...
chpwd () {
	local dirs
       ...
       awk 'NR == 2 {printf("%s", "[0;33m")}
            {print}' ORS=' '`"
# UCOLOR="[1;`[ $UID = 0 ] && 41;37 || 40;32"
PS1="$CTVIEW
[0m`date +%R`[0m [1;`[ $UID = 0 ] && echo '41;37' || echo '40;32'`m\u@\h[0m [1;33m$dirs
\[[1;36m\]\!\[[0m\] \[${CONNECTION_COLOR}\]${CONNECTION}\[[0m\]${CONNECTION_END}${SCRIPTING_MSG}b\\$ "
}
