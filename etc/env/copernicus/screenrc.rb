...
caption :always, '"%{= wk}%-Lw%{= bw}%n%f %t%{-}%+Lw %{= wk}%="'
...
hardstatus      :on
hardstatus      :lastline
hardstatus      :string, '"%{=br yk}%m/%d %c %{= dd} %-=%{-} %H %l"'
...
# bindkey(:screen,        'Alt-F1 == F1',             shift(:F1 ))     if copernicus
bindkey(stuffln(:exit), 'F12: exit',               Keymap[:F12])
# bindkey(stuffln(:exit), 'Alt-F2: == F2',            shift(:F2 ))     if copernicus
...
$initialWindows.downto(3) {|num| new_screen(num)}
