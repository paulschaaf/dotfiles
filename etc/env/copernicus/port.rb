...
$Legalize = [].instance_eval {
  ...
  # terminals
  push *%w(aterm eterm rxvt screen_term xterm vt100)
...
def resolve_host_aliases(*aliases)
  aliases.inject(Array.new) {|coll, e|
    value = $Hostname_Aliases[e]
    coll << value.to_sym if value
    coll
  }
end
...
class Symbol
  def resolvedHostname
    resolve_host_aliases(self)[0] || self
  end
...
$Servers = `hosts`.chomp.split.inject(HostNameList.new) {|coll, e| coll << e.to_sym}
...
class Command
  ...
  def with_env!(*env);         @env.push(*env);       self;  end
  def with_args!(*new);        @args.push(*new);      self;  end
  def with_flags!(*new)
...
class ShellScriptMaker
  attr_reader   :_sourceCode, :host
  attr_accessor :env, :packages_header, :shell, :shellVars, :showInfo, :use_color

  def ShellScriptMaker.legalize(*args)
    #dump(args)
    self.ignoreMethod(*args)
  end

  def ShellScriptMaker.uname_of(machine='')
    `shortUname #{machine.to_sym.resolvedHostname}`.chomp
  end

  legalize(*$Legalize)
  legalize(*$Servers.collect {|e| e.to_s})
  legalize(*self.uname_of('-a').downcase.split(' '))

  def ShellScriptMaker.default
    self.for()
  end

  def ShellScriptMaker.readFrom(aHash)
  ...
  def uname
    @uname ||= self.uname_of(self.host)
  end

  def uname=(an_uname)
    @uname = an_uname unless an_uname.empty?
  end
  ...
  def proper_term?(a_term)
    !(a_term.empty? || a_term == 'screen')
  end

  def correct_term(*termlist)
    termlist << ENV['TERM']
    termlist.detect {|e| self.proper_term?(term)} || self.default_term
  end
  ...
  def initialize(shell, args)
    ...
    allowMethod(self.host.slice(/[^.]*/).downcase)
    allowMethod(self.uname.gsub(/-/, '_').gsub(/_NT_.*/, '').downcase)
    allowMethod(self.term)
...
#         puts $/, "loc =   #{loc.inspect}"
  ...
  def ShellScriptMaker.basicEmitterCode(name, args={})
  ...
  def ShellScriptMaker.simpleEmitter(*names)
    names.collect {|name|
  ...
  def ShellScriptMaker.fixedLabelEmitter(*names)
    names.collect {|name|
  ...
  def ShellScriptMaker.emitterWithDefault(*names)
    names.collect {|name|
  ...
  def ShellScriptMaker.labeledMacro(*names)
    names.collect {|name|
      emitMethodName="emit_#{name}"
...
#   def ShellScriptMaker.delimitedMacro(name, lineHeading=name, separator="\n#{lineHeading}")
#     code = <<-CODE
...
#   end
  ...
  def ShellScriptMaker.delimitedMacro(name, separator=' ', label=name)
  ...
  def echoListFunction(name, varname=nil)
    function(name, Array.join(statement_sep) {|code|
               ...
               when :rubylib
                 #cmd  = 'echoList'
                 list = <<CODE
`ruby -e "print $:.join(':'); %w(#{args}).each {|e| print %Q< \#{e} \#{e}.rb>}"`
CODE
                 args = ''
               end
...
#   end
  ...
  def shell_not_exists(default=nil, &block)
    self.emitCode(block, default) {|cond| self.emit_shell_not_exists(cond)}
  end
  ...
  def emit_bindkey(args)
    Array.join {|answer|
      answer.push('# ') unless args[:keystroke]
