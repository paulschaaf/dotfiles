...
shell_if_exists(:TERM) if csh
  shell_if_eql(:TERM, :screen)
    ...
    setenv(:TERM,     :xterm) unless linux || cygwin
  shell_endif
shell_endif if csh
...
if package?(:cleartool)
  view_names = (%w{aix dec hp linux rhl sgi sol sun win} -
                {
                  aix      => %w{aix},
                  cygwin   => %w{win},
                  hpux     => %w{hp},
  ...
  shell_alias(:ct, :cleartool)

  function(:lsv) {
    sh %Q{
      if [ "$1" = "-h" ] || [ "$1" = "--help" ]; then
        echo "$0 [version]"
        echo "List all clearcase views for this uname. If version is supplied,"
        echo "display only those with that version number in the name."
        return
      fi
      local version
      if [ -z "$1" ]; then
        ...
        version='$0 ~ /'`echo $1 | sed 's/\\(.\\)/\\1[^0-9]*/g'`'/ && '
        shift
      fi
      ct lsview | awk $version'$(NF-1) !~ /#{view_names}/'
    }
  }

  shell_alias(:lsview, 'ct lsview')

  if package?(:versant)
    shell_alias(:v5081, "ctset #{vsnt_5081}") if vsnt_5081
    shell_alias(:v5082, "ctset #{vsnt_5082}") if vsnt_5082
    shell_alias(:v605,  "ctset #{vsnt_605}")  if vsnt_605
...
# -====== ls Aliases
  ...
  if package?(:ls_gnu)
    ...
    if cygwin
      $ls.add_flags('ignore nohup.out')
      $ls.add_flags('ignore hiberfil.sys')         # Hidden system file in root of c:
      $ls.add_flags('ignore pagefile.sys')         # Hidden system file in root of c:
      $ls.add_flags('ignore HKEY_PERFOMANCE_DATA') # In /proc/registry
    end
  end
...
function(:edit) {
  case $Editor
  # when :winclient
  #   comment('1) If xemacs is not already running:',
  #           '   a) Start it with no args (thus avoiding the 'flicker loop' startup bug)',
  #           '   b) Pause to give it time to start (so that winclient can find the process)',
  #           '2) Call winclient, passing any arguments to the running process)')
  #   any "ps|grep >&/dev/null xemacs || ((xemacs &); sleep 0.5)\n\twinclient #{args} &"
  when :gnuclient
    any "(#$Editor -f 'raise-frame' -q #{args} 2>/dev/null || xemacs #{args}) &"
  else
    any "$EDITOR #{args} &"
  end
}
...
unless ftp
  if ssh_server
    $remote_ssh_servers.each {|e|
      ...
      if e == :ftp
        args += ['-t', 'exec gnu/zsh ${*:--l}']
        sshToFunction(:vftp, *args)
      else
    ...
    unless home_desktop
      if_defined(:SSH_CLIENT)
        sshToFunction(resolve_host_aliases(:home_desktop)[0], '-C ${SSH_CLIENT/ */}')
      shell_endif
    end
...
if package?(:cvs)
  ...
  setenv(:CVSEDITOR, ($Editor == :winclient) ? :gnuclient : $Editor)
...
if package?(:java_sdk)
  setenv(:JAVA_HOME) {
    sunos '/net/vp/mvp/vogon/java/j2sdk1_3_1_04'
    linux '/usr/java/current'
  }
end
...
appendPath %w(/opt/bin /opt/$USER/firefox /opt/$USER/thunderbird /opt/cxoffice/bin) if owned
...
#    ) if zsh
  ...
  setdirs =
    if csh
      ...
      %Q{local dirs
       dirs="`builtin dirs #{"| tr -d '\\012'" if sunos} |
...
if cygwin
  ...
  setenv(self.path_varname, %Q<`echo $#{self.path_varname} | sed \'s~:/cygdrive~:~g\'`>)
